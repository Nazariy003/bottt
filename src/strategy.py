...
from config.settings import STRATEGY_CONFIG, TRADING_CONFIG, INDICATORS_CONFIG
...
class LightningVolumeStrategy:
        ...
        try:
            ...
            if market_regime.get('is_trending', False):
                return {
                    ...
                    'adx_thresh': self.strategy_config.get('trending_adx_thresh', 25),
                ...
                return {
                    'min_conf_long': self.strategy_config.get('ranging_min_conf_long', 3),
                    'min_conf_short': self.strategy_config.get('ranging_min_conf_short', 3),
    ...
    def _calculate_lightning_volume_boost(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if super_volume_surge:
                ...
                tp_multiplier_boost = 1 + (super_volume_boost / 100)
                ...
                tp_multiplier_boost = 1 + (vol_surge_boost / 100)
    ...
    def analyze_signals(self, symbol: str, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            base_buy_signal = (long_confirmations >= adaptive_params['min_conf_long'] and 
                              ...
                              adx_value_calc > adaptive_params['adx_thresh'])
            ...
            if base_buy_signal:
                ...
                signal_data_out = {
                    ...
                    'stop_loss': 0, 
                    'take_profits': [],
                    'reason': f'Недостатньо підтверджень (L:{long_confirmations}/S:{short_confirmations}, ADX:{adx_value_calc:.1f} vs {adaptive_params["adx_thresh"]})'
                }
    ...
    def _create_buy_signal(self, symbol: str, close_price: float, atr: float, 
                      ...
                      long_confirmations: int, volume_boost_info: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.strategy_config.get('use_triple_partial_tp', True):
                ...
                if abs(total_percent - 100.0) > 0.1:
                    ...
                    first_partial_percent = (first_partial_percent / total_percent) * 100
                    second_partial_percent = (second_partial_percent / total_percent) * 100
                    third_partial_percent = (third_partial_percent / total_percent) * 100
                ...
                final_tp_price = close_price + (dynamic_tp_mult * atr)
                take_profits_list = [
                    {'price': final_tp_price, 'percentage_to_close': 100.0, 'type': 'final'}
                ]
    ...
    def _create_sell_signal(self, symbol: str, close_price: float, atr: float, 
                    adaptive_params: Dict, dynamic_tp_mult: float, 
                    short_confirmations: int, volume_boost_info: Dict) -> Dict[str, Any]:
    ...
    def update_risk_management(self, trade_profit: float): 
        ...
        try:
            ...
            reduction_amount = self.consecutive_losses * (reduction_percent / 100.0) 
