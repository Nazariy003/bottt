...
from config.settings import TRADING_CONFIG, INDICATORS_CONFIG, STRATEGY_CONFIG
...
class DataPreprocessor:
    ...
    def __init__(self, chunk_size: int = 50, max_memory_usage_mb: int = 200):
        ...
        self.chunk_size = chunk_size  # Розмір чанку для обробки
        self.max_memory_usage_mb = max_memory_usage_mb
        self.critical_memory_usage_mb = max_memory_usage_mb * 1.5  # Critical threshold at 300MB
                ...
                for frame in [sys._getframe(i) for i in range(1, min(4, sys.getrecursionlimit()))]:
                    try:
                        for var_name, var_value in list(frame.f_locals.items()):
                            if hasattr(var_value, '__len__') and len(var_value) > 100:
            ...
            if psutil:
                try:
                    ...
                    if memory_freed > 0:
                        ...
                        effectiveness = (memory_freed / self._memory_usage_before_cleanup) * 100
                        ...
                        self.logger.warning(f"Очищення пам'яті виконано, але пам'ять не звільнено: {', '.join(cleanup_actions) if cleanup_actions else 'нічого не очищено'}")
                except:
                    if cleanup_actions:
        ...
        try:
            for symbol in self.trading_config['trade_pairs']:
                ...
                if should_update:
                    ...
                    num_candles_for_recalc = min(150, INDICATORS_CONFIG.get('slow_ma', 21) + 100)
        ...
        try:
            ...
            if integrity_ok:
                ...
                min_candles_for_strategy = max(
                    TRADING_CONFIG.get('min_candles_for_strategy', 100),  # Збільшено з 50 до 100
    ...
    def _validate_indicators_completeness(self, indicators_data: Dict, expected_length: int, symbol: str) -> List[str]:
        """
        ✅ ДОДАНО: Валідація повноти індикаторів
        ...
        for indicator_name, values in indicators_data.items():
            if values is None or len(values) == 0 or len(values) != expected_length:
                missing_indicators.append(f"{indicator_name}(len:{len(values) if values else 0})")
                continue
                
            # Перевіряємо кількість пропущених значень
            nan_count = sum(1 for v in values if pd.isna(v) or np.isinf(v))
            if nan_count > expected_length * 0.1:  # Більше 10% пропусків
                missing_indicators.append(f"{indicator_name}({nan_count} NaN)")
        
        if missing_indicators:
            self.logger.debug(f"Індикатори з пропусками для {symbol}: {missing_indicators}")
    ...
    def _get_default_indicator_value(self, indicator_name: str):
        ...
        if indicator_name in boolean_indicators:
            ...
            return 50.0  # Нейтральне значення RSI
