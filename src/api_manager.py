...
class BybitAPIManager:
    ...
    async def _get_http_session(self) -> aiohttp.ClientSession:
        ...
        if (self._session is None or 
            ...
            connector = aiohttp.TCPConnector(
                limit=100,  # Максимум одночасних з'єднань
                limit_per_host=20,  # Максимум з'єднань на хост
    ...
    async def _rate_limit_check(self):
        ...
        recent_failures = sum(1 for cb in self._circuit_breakers.values() 
                             if cb.failure_count > 0 and time.time() - cb.last_failure_time < 300)
    ...
    async def get_multiple_klines_concurrent(self, symbols: List[str], interval: str, limit: int = 1000) -> Dict[str, List[Dict]]:
        """Паралельне отримання klines для декількох символів"""
        semaphore = asyncio.Semaphore(5)  # Обмежуємо кількість одночасних запитів
    ...
    async def get_execution_history(self, symbol: str = None, limit: int = 50,
    ...
    async def analyze_position_from_execution_history(self, symbol: str,
        ...
        try:
            ...
            if not all_executions and is_demo_mode:
                ...
                if api_position_data:
                    for pos_item in api_position_data:
                        if pos_item.get('symbol') == symbol and float(pos_item.get('size', "0")) > 0.0000001:
                ...
                if active_api_pos:
                    ...
                    aggregated_position = {
                        ...
                        'size': float(active_api_pos.get('size', 0)),
                        'avg_price': float(active_api_pos.get('avgPrice', 0)),
                        'latest_price': float(active_api_pos.get('markPrice', active_api_pos.get('avgPrice', 0))),
                        'total_fees': 0.0,
                        'executions_count': 0,
                        ...
                        'unrealized_pnl': float(active_api_pos.get('unrealisedPnl', 0))
                    }
            ...
            tolerance = TRADING_CONFIG.get('sync_tolerance_qty', 0.0000001)
    ...
    async def reconcile_position_with_history(self, symbol: str,
        ...
        З покращеним обробленням Demo режиму.
            ...
            if not history_position:
                ...
                if abs(local_qty) > qty_tolerance:
                    ...
                    if diagnostics.get('last_closing_order_details'):
                        ...
                        close_price_for_pnl_calc = float(last_order.get('avgPrice', local_entry_price))
